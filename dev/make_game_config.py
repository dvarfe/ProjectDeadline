...
# Events
event0 = 'player.get_card( %s )'
event1 = 'player.free_hours_today += %s'  # Проверка корректности происходит вовне
event2 = 'player.free_hours_today -= %s'  # Проверка корректности происходит вовне
...
with open('../Deadline/game_config.json', 'w') as f:
    data = {
        ...
        'effects': [
            effect('e0', 'Кофе', 'Выпить кружку кофе', 'coffee.png', 1, 0, False, [event1 % '6'], [event2 % '6'], []),
        ],
        'tasks': [
            task('t0', 'Матан',          'Обычное задание по матану',                     'math_hw.png', 3, 3, 3, -6,
                 [],                 []),
            task('t1', 'ML',             'Research-задание по ML',                        'cs_hw.png',   5, 7, 5, -5,
                 [event0 % '"tc2"'], []),
            task('t2', 'Кросс-проверка', 'Нужно проверить 3 чужих решения домашки по ML', 'cs_hw.png',   1, 7, 0, -5,
                 [],                 []),
        ],
        'task_cards': [
